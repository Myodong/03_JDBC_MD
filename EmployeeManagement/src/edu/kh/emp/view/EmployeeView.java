...
public class EmployeeView {
	...
	public void selectEmpId() {
		...
		// 입력받은 사번을 DAO selectEmpId() 메서드로 전달하여
		// 조회된 사원 정보 반환 받기
		Employee emp = dao.selectEmpId(empId);
		
		printOne(emp); // 조회 결과반환
	}
	...
	public void selectEmpNO() {
		...
		Employee emp = dao.seletEmpNo(empNo);
		printOne(emp);
	}
	...
	public void deleteEmployee() {
		...
		char input =sc.next().toUpperCase().charAt(0); // toUpperCase() 대소문자 구분없이 입력받음 (모두 대문자로 변환)
